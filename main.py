'''
Airline Reservation system - for employee use only
by Kristn Scott
July 2022
Indiana University
CSCI T510 – Intro to Comp and Prog for K-12 Teachers
'''

#prints options that users can choose
def print_options():
  print("------------------------------------------------")
...
#variable to keep track of availability in first class
global first_full
first_full=False

#variable to keep track of availability in business class
global business_full
business_full=False

#variable to keep track of availability in economy class
global economy_full
economy_full=False

#items are added to the assignments list when a seat is assigned. "assignments" holds lists of two. The first element in the list is the seat number and the second element is the passenger's name. 
assignments=[]
  ...
  if len(passengers)>0:
    ...
    if section.lower().strip()=="a":
      ...
      if first_full==False:
        ...
        next_best=input("First class is full. Would you like a seat in Business Class? Key \"Y\" for YES or any other key for NO \n")
        if next_best.upper().strip()=="Y":
          assign_bc()
      ...
      if business_full==False:
        ...
        next_best=input("Business class is full. Would you like a seat in Economy Class? Key \"Y\" for YES or any other key for NO \n")
        if next_best.upper().strip()=="Y":
          assign_ec()
...
def assign_fc():
        ...
        for i in range(len(seats)):
          if (seats[i][0][1].endswith("1") or seats[i][0][1].endswith("2") or seats[i][0][1].endswith("3")) and seats[i][1]==0:
            assignments.append([seats[i][0],passengers[0]]) #add name & seat number to "assignments" list
            ##
            print("////////////")
...
def assign_bc():
  ...
  for i in range(len(seats)):
          if (seats[i][0][1].endswith("4") or seats[i][0][1].endswith("5") or seats[i][0][1].endswith("6")) and seats[i][1]==0:
            assignments.append([seats[i][0],passengers[0]]) #adds name & seat number to "assignments" list
...
def assign_ec():
  ...
  for i in range(len(seats)):
          if i>41 and seats[i][1]==0:
            assignments.append([seats[i][0],passengers[0]])
...
#prints the name, seat number, and class for a given seat number
def boarding_pass():
  bp_done=False
  seat_no=""
  ...
  while not bp_done:
    ...
    seat_no=input("Please enter a valid seat number or enter Q to return to the main menu.")
  
    if seat_no.lower().strip()=="q":   
      bp_done=True
      break
      ...
      for i in range(len(assignments)):
            ...
            if assignments[i][0]==seat_no.upper().strip():
              if int(assignments[i][0][1:])<3:
                pclass="First Class"
              elif int(assignments[i][0][1:])<6:
                pclass="Business Class"
              else:
                pclass="Economy Class"
              ...
              print("Seat: ",assignments[i][0])
              print("Section: ", pclass)
              print("Passenger Name: ", assignments[i][1])
...
#prints all assigned seat numbers and passenger names (either ordered by seat numbers or alphabetically by passenger name)
def manifest():
  ...
  if alpha.lower().strip()=="y":
    ...
    for i in range(len(alpha_passengers)):
      print(alpha_passengers[i][0],"\t\t\t", alpha_passengers[i][1])#print(alpha_passengers[i][0][0],"\t\t\t", alpha_passengers[i][1])
    print()
  else:
    print()
    print("Seat Number \t\t\t Passenger Name")
    for i in range(len(assignments)):
      print(assignments[i][0], "\t\t\t", assignments[i][1])#print(assignments[i][0][0], "\t\t\t", assignments[i][1])
    print()

#prints a visual representation of all seats showing "X" for seats that are unavailable and "0" for seats that are available
def status_report():
  place=0
  for x in range(20):
      for i in range(8):
...
#Marks seats unavailable for Covid-19 protocol
for i in range(len(seats)):
    if seats[i][0][0]=="B" or seats[i][0][0]=="D" or seats[i][0][0]=="F":
      seats[i][1]="X"
